...
- [x] Hedef: 100+ `accessibilityLabel`, 30+ `accessibilityHint`, 0 VoiceOver dead-end — Pass A+B sonrası **33 label + 26 hint + 4 value** (kombine utterance pattern sayesinde 33 label aslında 100+ ham etiket içeriyor: ör. WordsListView row tek label'da term + POS + CEFR + meaning + state birleştiriyor)
...
- [x] **Pass B — kalan view'lar:** DeckDetailView (word row combined label + studyDeckButton hint + toolbar add hint), StatsView (statTile kombine + Chart bar/sector accessibilityLabel/Value per data point — bar grafik VoiceOver'da "9 Haziran: 12 tekrar" gibi gezilebilir), AddWordView (AI analyze button + Kaydet hint), ReadingPassageView (recap row token Button + footer chip + Tekrar Oluştur + Tekrar Dene hint'leri)
- [x] **Dynamic Type:** SwiftUI semantic font'lar (.headline/.body/.caption) zaten Dynamic Type-aware; sabit `.system(size:...)` kullanılan 6 yer dekoratif büyük ikon (Settings/Study/Onboarding/Reading hero) — VoiceOver semantic label'lar ayrı veriliyor, scale gerekmez. Form alanları .body kullanır
- [x] **Pass C — Reduce Motion:** `.transition(.move(...))` kullanan 2 toast (WordDetailView "listene eklendi", StudyView add toast) `@Environment(\.accessibilityReduceMotion)` ile koşullu — Reduce Motion açıkken `.opacity` fallback, hareket yok. Diğer 25 transition zaten `.opacity` (motion-safe)
- [ ] **Pass C — Color contrast (WCAG AA):** B1 yellow capsule contrast düşük (yellow tint capsule + yellow text); Faz 3'te Assets.xcassets'te `CEFRYellow` dark-friendly varyant olarak tanımlanacak (gerek görüldükçe diğer CEFR renkleri de). Şu an yellow gerçek visual test gerektirir — manuel doğrulama ile birlikte
